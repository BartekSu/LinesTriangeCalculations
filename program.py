...
from PyQt5.QtWidgets import QApplication, QFileDialog, QMessageBox, QSlider, QTableWidgetItem #pylint: disable=no-name-in-module, import error, bad-option-value
...
class program_Sura(wnd, cls):
    ...
    def on_RysujB1_pressed(self):
        ...
        if self.X1_1.text() == "" or self.X2_1.text() == "" or self.Y1_1.text() == "" or self.Y2_1.text() == "":
            ...
            self.mpl.czysc()
            self.mpl.rysuj(float(self.X1_1.text()), float(self.X2_1.text()), float(self.Y1_1.text()), float(self.Y2_1.text()))

    def on_RysujB2_pressed(self):
        print('Rysuję')
        if self.X1_2.text() == "" or self.X2_2.text() == "" or self.Y1_2.text() == "" or self.Y2_2.text() == "":
            print("Wpisz liczby we wszytkie okienka")    
        else:
            self.mpl_2.czysc()
            self.mpl_2.rysuj_2(float(self.X1_2.text()), float(self.X2_2.text()), float(self.X3_2.text()), float(self.Y1_2.text()), float(self.Y2_2.text()), float(self.Y3_2.text()))
    ...
    def on_ObliczB2_pressed(self):
        if self.X1_2.text() == "" or self.X2_2.text() == "" or self.X3_2.text() == "" or self.Y1_2.text() == "" or self.Y2_2.text() == "" or self.Y3_2.text() == "":
            print("Wpisz liczby we wszytkie okienka")
        else:
            ...
            p = (1/2)*(D12 + D23 + D31)
            pH = math.sqrt((p*(p-D12)*(p-D23)*(p-D31)))
            self.pL.setText(str(round(pH,2)))
           
           
            #Obliczenie pola za pomocą tw sinusa
            p = math.acos((pow(D12,2)+pow(D23,2)-pow(D31,2)) / (2*D12*D23))
            pS = (1/2)*D12*D23*math.sin(p)
            self.pL2.setText(str(round(pS,2)))
